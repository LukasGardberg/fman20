...
S_feats = zeros(nbr_feats, 100); % 7, since I use 7 features -- change appropriately!
for i = 1 : numel(S)
...
preds = predict(classification_data, S_feats');

% % only for nn
% [m, pred_c] = max(preds');
% sum(pred_c - y == 0) / length(y)
